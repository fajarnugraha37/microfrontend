...
export function veeValidateCompatPlugin() {
    ...
    function reset(name, scope) {
        ...
        if (scope != null) {
            ...
            // });
            delete errorsState[GLOBAL_SCOPE];
        }
    }
    ...
    const errorsHelper = {
        first(name, scope) {
            ...
            // }
            const list = errorsState[GLOBAL_SCOPE]?.[name];
            if (list && list.length) return list[0];
    ...
    /**
     * @param {{ getAttribute: (arg0: string) => any; }} el
     * @param {{ arg: any; }} binding
     */
    ...
    /**
     * @param {HTMLInputElement | HTMLTextAreaElement | HTMLSelectElement} el
     * @param {import("vue").DirectiveBinding<any, string, any>} binding
     */
    function registerField(el, binding) {
        ...
        /**
         * @type {{ event: string; handler: (e: Event) => void; }[]}
         */
        const listeners = [];
